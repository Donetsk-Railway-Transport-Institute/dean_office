...
Change_Curr_Year::Change_Curr_Year(
        startup_data *s_data,
        QWidget *parent):QDialog(parent), s_d(s_data){
    setWindowTitle("Изменение учебного года");
    resize(237, 134);
    ...
    label->setText("Введите учебный год");
    lineEdit = new QLineEdit(this);
    lineEdit->setGeometry(QRect(60, 40, 111, 20));
    lineEdit->setText(s_d->xrw.curr_year);
...
Order_Recognition::Order_Recognition(
    ...
    setWindowTitle("Распознавание приказов о зачислении");
    resize(662, 671);
    buttonBox = new QDialogButtonBox(this);
    buttonBox->setObjectName(QString::fromUtf8("buttonBox"));
    buttonBox->setGeometry(QRect(300, 620, 341, 32));
    buttonBox->setOrientation(Qt::Horizontal);
    buttonBox->setStandardButtons(QDialogButtonBox::Cancel|QDialogButtonBox::Ok);
    textEdit = new QTextEdit(this);
    textEdit->setObjectName(QString::fromUtf8("textEdit"));
    textEdit->setGeometry(QRect(10, 10, 641, 591));
...
void Order_Recognition::control_BD(){
    int counter = 0;
    textEdit->append("Проверка целостности баз данных.");
    textEdit->append(s_d->xrw.curr_year+" учебный год.\n");
    for(auto gr:s_d->groups){
        int count_gr = 0;
        textEdit->append(gr[0]);
        for(auto st:s_d->students[gr[0]]){
            if(st[0].indexOf(" ")==0){
                textEdit->append(st[0]+" пробел");
            } else textEdit->append(st[0]);
            counter++;
            count_gr++;
        }
        textEdit->append(QString("\t\tИтого - %1 студентов\n").arg(count_gr));
    }
    textEdit->append(QString("Общее количество - %1 студентов").arg(counter));
}

void Order_Recognition::open_all(){
    ...
    if(!dir.isEmpty()){
        ...
        years = d.dirName().split("(")[1].split(")")[0];
...
void Order_Recognition::open_doc(const QString &filename){
...
//    qInfo("%s",student_file_name.toUtf8().constData());
    ...
    rec_students(tmp_text.toPlainText(), student_file_name);
    tmp_dir.removeRecursively();
}

void Order_Recognition::rec_students(const QString &data, const QString &s_f_name){
